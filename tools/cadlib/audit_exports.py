...
docs={}
native_sets=[(prefix+'_Complete.FCStd','objects'),(prefix+'_Exploded.FCStd','exploded_objects')]
if (out/(prefix+'_Closed.FCStd')).exists():native_sets.append((prefix+'_Closed.FCStd','closed_objects'))
for filename,key in native_sets:
    d=App.openDocument(str(out/filename));d.recompute();docs[key]=d
    rows=manifest['objects' if key=='closed_objects' else key]
