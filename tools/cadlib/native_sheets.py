...
def create_native_sheets(model):
    ...
    drawing_date=html.escape(json.loads((draw/'content.json').read_text())['content']['date'])
    dimension_note='全部尺寸近似，内部为布局示意' if model.profile.get('envelope_kind')=='approximate' else '局部尺寸近似，内部为布局示意'
    ...
    for page in pages:
        ...
        template.write_text(f'''<svg xmlns="http://www.w3.org/2000/svg" xmlns:freecad="http://www.freecad.org/wiki/index.php?title=Svg_Namespace" width="420mm" height="297mm" viewBox="0 0 420 297"><g fill="none" stroke="#504e49" stroke-width=".2"><rect x="8" y="8" width="404" height="281"/><path d="M 8 264 H 412 M 252 264 V 289 M 338 264 V 289"/></g><g font-family="Noto Serif CJK SC" fill="#141413"><text x="12" y="19" font-size="7.0">{title}</text><text x="12" y="278" font-size="4.0">{html.escape(model.profile['title'])} · CAD Study</text><text x="12" y="285" font-size="3.2">{dimension_note}</text><text x="258" y="274" font-size="3.2">单位 mm · 比例见视图</text><text x="258" y="283" font-size="3.2">{drawing_date}</text><text x="344" y="274" font-size="3.0">{prefix}-DWG-{number:02d}</text><text x="344" y="283" font-size="3.2">A3 · {number} / {len(pages)}</text></g></svg>''')
