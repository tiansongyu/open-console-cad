...
def official_logo(key,width,pos,assembly='Tablet',layer=-5,material='white',orientation=None):
    ...
    if 'LOGO_MASTER' not in globals():
        exec(compile((ROOT/'scripts/svg_exact.py').read_bytes(),'svg_exact.py','exec'),globals())
        LOGO_MASTER=svg_faces(str(ROOT/'references/switch_mark.svg'))
        bb=LOGO_MASTER.optimalBoundingBox(False,False)
        LOGO_MASTER.translate(-bb.Center)
    shape=LOGO_MASTER.copy();m=App.Matrix();m.A11=m.A22=m.A33=width/shape.optimalBoundingBox(False,False).XLength;shape=shape.transformGeometry(m)
...
def joy_body(key,label,side,depth,z,material):
    x=side*117.5
    body=rounded_body(D,key+'Blank',label+' · 原坯',37,116,0,depth,(x,0,z),color=PALETTE[material],expr={'Height':'Parameters.JoyHeight'})
    pad=body.Tip
    names=[f'Edge{i+1}' for i,e in enumerate(pad.Shape.Edges) if e.BoundBox.ZLength>depth-1e-7 and abs(e.BoundBox.XMin-side*18.5)<1e-7]
    outer=body.newObject('PartDesign::Fillet',key+'OuterCurve');outer.Base=(pad,names);outer.Radius=29.0;body.Tip=outer;D.recompute();pad.Visibility=False
    names=[f'Edge{i+1}' for i,e in enumerate(outer.Shape.Edges) if e.BoundBox.ZLength>depth-1e-7 and abs(e.BoundBox.XMin+side*18.5)<1e-7]
    inner=body.newObject('PartDesign::Fillet',key+'JointCurve');inner.Base=(outer,names);inner.Radius=1.8;body.Tip=inner;D.recompute();outer.Visibility=False
    return register(body,key,'JoyLeft' if side<0 else 'JoyRight',4 if z>0 else -4,material)
