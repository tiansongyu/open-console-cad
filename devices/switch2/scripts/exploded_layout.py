...
def exploded_offset(key,src):
    ...
    if a in ['Tablet','Display','TabletInternal']:
        ...
        if key in ['Kickstand','StandFoot-1','StandFoot1','StandHinge-1','StandHinge1','StandPin-1','StandPin1']:z=-710
        if key=='RearEMIShield':z=-470
        if key.startswith('Battery'):z=-240
        if key.startswith(('Fan','CoolingFin')) or key in ['HeatSpreader','CopperHeatPipe','SoCThermalPad','HeatPipeBridge','ThermalFilm']:z=-350
        z={'AntiShatterFilm':570,'DisplayGlass':500,'FrontBezel':380,'LCDModule':260,'DisplayBackplate':140}.get(key,z)
        return vec(0,0,z)
    if a.startswith('Joy'):
        left=a in ['JoyLeft','JoyMountL','JoyInternalL'];dx=-157 if left else 157
    ...
    if a in ['Dock','DockInternal']:
        ...
        if key.startswith('DockFrontGuide'):z=125
        if key.startswith(('DockPlug','DockUSBCPlug','DockGuidePin')):z=80
        if key=='DockFloatingCarriage' or key.startswith('DockSpring'):z=40
        return vec(-110,-300,z)
    if a=='Grip':
        dx=-83
        if key.startswith('GripHandle'):dx+=-32 if '-1' in key else 32
        elif key.startswith('GripLiner'):dx+=-18 if '-1' in key else 18
...
'exploded_joy_left':dict(assemblies=['JoyLeft','JoyMountL','JoyInternalL'],normal=(.9,-.15,1.2),target=(-260,-220,-15),span=250,size=(1800,1500)),
'exploded_joy_right':dict(assemblies=['JoyRight','JoyMountR','JoyInternalR'],normal=(-.9,-.15,1.2),target=(260,-220,-15),span=250,size=(1800,1500)),
