...
ROOT=Path(__file__).resolve().parents[1];DRAW=ROOT/'output/drawings'
...
assert len(pages)==12
ir['content']['summary']['claim']='本套图纸对应 Nintendo Switch 2 首发版详细 CAD 模型，公开包络与局部近似尺寸分别标注。视图由实体投影生成，用于装配观察与 FreeCAD 学习，内部是主要模块布局示意。'
...
for key,value in {'{{文档标题}}':ir['content']['title'],'{{作者}}':'CAD Study','{{摘要}}':ir['content']['summary']['claim'],'{{关键词}}':'Nintendo Switch 2, FreeCAD, 工程图, 爆炸图, CAD'}.items():head=head.replace(key,value)
...
for i,page in enumerate(pages):
 ...
 body.append(f'''<section class="drawing-sheet" id="sheet-{i+1:02d}"><div class="drawing-header"><div class="eyebrow">{html.escape(ir['content']['subtitle'])} <span style="float:right">SW2-DWG-{i+1:02d} / 12</span></div><h1>{html.escape(page['title'])}</h1><p class="claim">{html.escape(claim)}</p></div><figure class="drawing-figure">{svg}</figure><div class="drawing-notes">{''.join('<p>'+html.escape(x)+'</p>' for x in notes)}</div><div class="title-block"><div><div class="title">Nintendo Switch 2 详细模型图纸</div><div>局部尺寸为本模型近似值 · 内部为布局示意</div></div><div>单位 mm · 比例见各视图<br>CAD Study · 2026.09.08</div><div>图号 SW2-DWG-{i+1:02d}<br>第 {i+1} / 12 页 · A3</div></div></section>''')
...
out=DRAW/'Switch2_Drawings.html';out.write_text(filled)
count=render_pdf(out,DRAW/'Switch2_Drawings.pdf');assert count==12,count
print(json.dumps({'pages':count,'html':str(out),'pdf':str(DRAW/'Switch2_Drawings.pdf'),'pipeline':'Kami render_pdf'},ensure_ascii=False))
