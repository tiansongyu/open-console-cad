...
D=App.openDocument(str(OUT/'Switch2_Complete.FCStd'));E=App.openDocument(str(OUT/'Switch2_Exploded.FCStd'))
...
def projection(keys,normal,up,center,exploded=False):
 objs=EC if exploded else C
 token=hashlib.sha256(json.dumps(["planar_strap_face_v1",keys,normal,up,center,exploded,[(k,objs[k].Shape.Volume,list(objs[k].Shape.optimalBoundingBox(False,False).Center)) for k in keys]]).encode()).hexdigest()[:20]
 if any(k in ['StrapLCord','StrapRCord'] for k in keys):token+='_cord_section_v2'
 p=CACHE/(token+'.svg')
 if p.exists():return p.read_text()
 q=rot(normal,up).inverted();parts=[]
 for k in keys:
  sh=objs[k].Shape
  # Use the exact upper planar face of strap frames in their orthographic front view.
  # Internal cord bores are below this face; HLR emits spurious untrimmed curves for them.
  if not exploded and tuple(normal)==(0,0,1) and k in ['StrapLFrame','StrapRFrame']:
   zmax=sh.optimalBoundingBox(False,False).ZMax
   faces=[f for f in sh.Faces if f.optimalBoundingBox(False,False).ZLength<1e-6 and abs(f.CenterOfMass.z-zmax)<1e-5]
   assert faces,k
   sh=Part.makeCompound(faces)
  if not exploded and tuple(normal)==(0,0,1) and k in ['StrapLCord','StrapRCord']:
   # A circular tube swept in the XY plane has its exact plan silhouette at its midplane.
   # Discretization avoids the HLR SVG serializer extending periodic curve seams.
   wires=sh.slice(App.Vector(0,0,1),3.5)
   assert len(wires)==2 and all(w.isClosed() for w in wires)
   polygon_wires=[]
   for wire in wires:
    pts=wire.discretize(Deflection=.015)
    if (pts[0]-pts[-1]).Length>1e-9:pts.append(pts[0])
    polygon_wires.append(Part.Wire(Part.makePolygon(pts)))
   sh=Part.makeFace(polygon_wires,'Part::FaceMakerBullseye')
  parts.append(sh)
 s=Part.makeCompound(parts);s.Placement=App.Placement(-q.multVec(App.Vector(*center)),q).multiply(s.Placement)
...
ext=keys_for(['Tablet','Display','JoyLeft','JoyRight','JoyMountL','JoyMountR'],False)
s=Sheet(1,'手持整机总装图',['首发版 BEE-001，图中最大厚度包含摇杆与扳机。','公开包络尺寸采用 Nintendo 资料，局部轮廓为本模型近似设计。'])
v=s.view('HandheldFront',ext,159,80,1,center=(0,0,6.95));s.text(159,151,'正面 · 1:1',anchor='middle')
s.dim(v,(-136,0,0),(136,0,0),'x',17,published=True);s.dim(v,(0,-58,0),(0,58,0),'y',16,published=True)
r=s.view('HandheldRight',ext,343,80,1,normal=(1,0,0),center=(0,0,9.35));s.text(343,151,'右视 · 1:1',anchor='middle')
s.dim(r,(0,0,-6),(0,0,24.7),'x',162,published=True)
t=s.view('HandheldTop',ext,159,183,1,normal=(0,1,0),up=(0,0,1),center=(0,0,9.35));s.text(159,211,'顶视 · 1:1',anchor='middle')
s.text(308,180,'主机主体厚度 13.9 mm',3.2);s.text(308,188,'所有尺寸单位：mm',3.2);s.text(308,196,'局部近似值用 ≈ 标识',3.2);s.save()
# 02 / front and rear are reduced to 0.9:1 to fit the larger tablet.
tablet=keys_for(['Tablet','Display'],False)
s=Sheet(2,'主机结构尺寸',['显示区按 7.9 英寸、16:9 建立，主机单体宽 198 mm 为比例近似。','宽 U 形支架、双 USB-C、卡槽与进排风口均保留在实体中。'])
f=s.view('TabletFront',tablet,104,71,.9,center=(0,0,6.95));b=s.view('TabletBack',tablet,300,71,.9,normal=(0,0,-1),center=(0,0,6.95))
s.text(104,137,'主机正面 · 0.9:1',anchor='middle');s.text(300,137,'主机背面 · 0.9:1',anchor='middle')
s.dim(f,(-99,0,0),(99,0,0),'x',12);s.dim(f,(0,-58,0),(0,58,0),'y',10,published=True)
g=bb(['DisplayGlass']);s.dim(f,(g.XMin,0,13.9),(g.XMax,0,13.9),'x',32)
s.dim(b,(-91.5,-27.2,0),(91.5,-27.2,0),'x',149)
kb=bb(['Kickstand']);s.dim(b,(0,kb.YMin,0),(0,kb.YMax,0),'y',210)
t=s.view('TabletTop',tablet,104,176,.9,normal=(0,1,0),up=(0,0,1),center=(0,0,6.95));u=s.view('TabletBottom',tablet,300,176,.9,normal=(0,-1,0),up=(0,0,1),center=(0,0,6.95))
s.text(104,198,'顶部：按键 / 排风 / USB-C / 耳机 / 卡槽',3.2,'middle');s.text(300,198,'底部：USB-C / 进风口',3.2,'middle')
s.dim(u,(0,-58,0),(0,-58,13.9),'y',392,published=True);s.save()
# 03 / controller exterior and button locations.
lkeys=keys_for(['JoyLeft','JoyMountL'],False);rkeys=keys_for(['JoyRight','JoyMountR'],False)
s=Sheet(3,'左右 Joy-Con 2 尺寸',['左右手柄具有相同的包络，摇杆与面按键的位置不同。','手柄宽度包含磁吸连接突起，最大厚度包含摇杆与后扳机。'])
l=s.view('JoyLeftFront',lkeys,56,80,1,center=(-115.3,0,9.35));r=s.view('JoyRightFront',rkeys,158,80,1,center=(115.3,0,9.35));inner=s.view('JoyLeftInnerRail',lkeys,259,80,1,normal=(1,0,0),center=(-115.3,0,9.35));back=s.view('JoyRightBack',rkeys,353,80,1,normal=(0,0,-1),center=(115.3,0,9.35))
for x,label in [(56,'L 正面'),(158,'R 正面'),(259,'L 磁吸面'),(353,'R 背面')]:s.text(x,145,label+' · 1:1',anchor='middle')
s.dim(l,(-136,0,0),(-94.6,0,0),'x',16,published=True);s.dim(l,(-118,-58,0),(-118,58,0),'y',20,published=True)
s.dim(r,(107.2,29.3,15.35),(128.8,29.3,15.35),'x',163,label='21.6')
s.dim(inner,(-118,0,-6),(-118,0,24.7),'x',163,published=True)
s.text(10,184,'摇杆帽 Ø17.1 · 面按键 Ø7.4 · 相邻方向中心距 10.8',3.2)
s.text(10,194,'L 摇杆中心：X=-118，Y=29，R 摇杆中心：X=118，Y=0',3.2)
s.text(10,204,'上述局部尺寸均为近似值，按当前模型坐标记录。',3.2)
s.save()
# 04 / six enlarged model-derived interface and mechanism details.
s=Sheet(4,'接口、磁吸与鼠标详图',['上下 USB-C 插座具有分开的金属壳、绝缘芯和触点。','磁吸靶片位于 SL/SR 侧键后方，两个手柄各有光学鼠标模块。'])
usbc=[k for k in C if k.startswith('USBLower')]
v=s.view('USBCDetail',usbc,65,49,5,normal=(0,-1,0),up=(0,0,1),center=(0,-58,6.95));s.text(65,80,'底部 USB-C · 5:1',anchor='middle')
us=bb(['USBLowerShell']);s.dim(v,(us.XMin,-58,7.1),(us.XMax,-58,7.1),'x',27);s.dim(v,(0,-58,us.ZMin),(0,-58,us.ZMax),'y',20)
v=s.view('HeadphoneDetail',['HeadphoneRim','HeadphoneContact'],192,49,5,normal=(0,1,0),up=(0,0,1),center=(50.2,58,7.1));s.text(192,80,'耳机插座 · 5:1',anchor='middle')
v=s.view('StickDetail',[k for k in C if k.startswith('JoyLStick')],315,49,2,normal=(0,1,0),up=(0,0,1),center=(-118,29,19.3));s.text(315,80,'加大摇杆 · 2:1',anchor='middle');s.dim(v,(-118,29,13.9),(-118,29,24.7),'y',356)
v=s.view_fit('MagneticSection',['ConsoleLinerL','ConsoleMagnetL1','JoyLSteelTarget1','JoyLSL'],(15,105,100,72),normal=(0,-1,0),up=(0,0,1));s.text(65,196,'磁吸连接端面 · 缩放详图',anchor='middle')
v=s.view('MouseDetail',['JoyRMousePCB','JoyRMouseSensor','JoyRMouseLens','JoyRMouseWindow'],192,151,7,normal=(.4,.3,1),center=(96.5,12.2,7));s.text(192,196,'光学鼠标模块 · 7:1',anchor='middle')
v=s.view('HapticDetail',['JoyLHapticCase','JoyLHapticMass','JoyLHapticCoil','JoyLHapticSpring0','JoyLHapticSpring1'],315,151,4.5,normal=(.3,-.3,1),center=(-113,-42,2.8));s.text(315,196,'HD Rumble 2 布局 · 4.5:1',anchor='middle');s.save()
...
v=section_view(s,'TabletSectionY0',keys_for(['Tablet','Display','TabletInternal']),198,51,1.8,0,(0,0,6.95));s.text(198,20,'A-A · 主机 Y=0 剖面 · 1.8:1',anchor='middle');s.dim(v,(0,0,0),(0,0,13.9),'y',18,published=True)
v=section_view(s,'JoyLeftSection',keys_for(['JoyLeft','JoyMountL','JoyInternalL']),105,146,4.3,29,(-115.3,29,11));s.text(105,78,'B-B · L 摇杆中心 · 4.3:1',anchor='middle')
v=section_view(s,'JoyRightSection',keys_for(['JoyRight','JoyMountR','JoyInternalR']),293,146,4.3,0,(115.3,0,11));s.text(293,78,'C-C · R 摇杆中心 · 4.3:1',anchor='middle');s.text(105,213,'剖切坐标 Y=29',anchor='middle');s.text(293,213,'剖切坐标 Y=0',anchor='middle');s.save()
...
ks=keys_for(['Tablet','Display','TabletInternal']);v=s.view_fit('TabletExploded',ks,(7,25,382,102),normal=(1.7,-.2,-1),exploded=True)
...
s=Sheet(7,'左右手柄分层爆炸图',['手柄前后壳、电池、PCB、摇杆及振动单元分层展示。','两侧均含鼠标传感器，右侧另含 NFC 天线和 C 聊天键。'])
l=s.view_fit('JoyLeftExploded',keys_for(['JoyLeft','JoyMountL','JoyInternalL']),(12,26,175,112),normal=(.9,-.15,1.2),exploded=True)
r=s.view_fit('JoyRightExploded',keys_for(['JoyRight','JoyMountR','JoyInternalR']),(209,26,175,112),normal=(-.9,-.15,1.2),exploded=True)
s.text(101,18,'Joy-Con 2 L',4,'middle');s.text(296,18,'Joy-Con 2 R',4,'middle')
...
s=Sheet(8,'底座结构尺寸',['含防滑脚的底座包络为 201 × 115 × 51.2 mm。','前后软垫限定主机插入槽，内部连接器和主板可在爆炸页中观察。'])
f=s.view('DockFront',dockkeys,116,73,1.0,center=(0,-208,0));r=s.view('DockSide',dockkeys,310,71,1.0,normal=(1,0,0),center=(0,-208,0));t=s.view('DockTop',dockkeys,109,175,1.0,normal=(0,1,0),up=(0,0,1),center=(0,-208,0))
s.dim(f,(-100.5,-208,0),(100.5,-208,0),'x',12,published=True);s.dim(f,(0,-265.5,0),(0,-150.5,0),'y',13.5,published=True);s.dim(r,(0,-208,-25.6),(0,-208,25.6),'x',151,published=True)
s.text(116,140,'正面 · 1:1',anchor='middle');s.text(310,140,'右视 · 1:1',anchor='middle');s.text(109,212,'顶视 · 1:1',anchor='middle')
s.text(230,171,'前后导垫间距：≈14.25 mm',3.2);s.text(230,182,'后部：USB-C 电源 / HDMI / LAN',3.2);s.text(230,193,'左侧：2 个 USB-A 接口',3.2);s.save()
# 09 / dock exploded view with service-part legend.
s=Sheet(9,'底座分层爆炸图',['后盖移开后，可单独观察接口板和接插件、散热风扇与浮动插头。','主机导垫与插头支座分别编号，外壳保持与总装页相同的几何。'])
v=s.view_fit('DockExploded',dockkeys,(10,18,376,117),normal=(1.5,-.2,-1),exploded=True)
for i,(key,label) in enumerate([('DockFront','前壳'),('DockFrontGuide-1','前导垫'),('DockUSBCPlug','插头'),('DockTower','后部壳体'),('DockPCB','接口板'),('DockBackCover','后盖'),('DockScrew1','后盖螺钉'),('DockFanHousing','主动散热风扇')]):
...
s=Sheet(10,'握把与腕带结构',['握把为非充电型外观研究，磁吸槽与手持控制器使用相同装配概念。','腕带包含开口框架、鼠标脚垫、导光窗、闭合织绳和圆形调节扣。'])
gkeys=keys_for(['Grip']);gb=bb(gkeys);gc=list(gb.Center)
v=s.view('GripFront',gkeys,77,82,1,center=gc);s.dim(v,(gb.XMin,gc[1],gc[2]),(gb.XMax,gc[1],gc[2]),'x',17,published=True)
...
s.text(10,181,'握把公开包络 '+f'{gb.XLength:.2f} × {gb.YLength:.2f} × {gb.ZLength:.2f} mm')
s.text(10,192,'腕带外壳：16.4 × 116 × 7 mm；织绳截面 ≈Ø1.56 mm'.replace('；','，'))
s.text(10,203,'握把及腕带框架采用日文官网尺寸，闭合绳路为近似摆放。');s.save()
# 11 / complete component roll-up and material key.
CN={'Display':'显示组件','Tablet':'主机外部结构','TabletInternal':'主机内部布局','JoyLeft':'左手柄外部','JoyRight':'右手柄外部','JoyMountL':'左手柄磁吸脊','JoyMountR':'右手柄磁吸脊','JoyInternalL':'左手柄内部','JoyInternalR':'右手柄内部','Dock':'底座外壳','DockInternal':'底座内部','Grip':'握把','StrapL':'左腕带','StrapR':'右腕带'}
s=Sheet(11,'零件与材料索引',['每个物理组件有独立 SW2 编号，可对照模型树和 COMPONENTS.csv。','多实体的标识字形与网格仍作为一个组件记录，组件数与实体数分别列出。'])
...
(16,'公开尺寸',["整机 272 × 116 × 13.9 mm，含摇杆/扳机最大厚 30.7 mm。","手柄含连接突起宽 41.4 mm，主机单体宽 198 mm 为近似。","底座 201 × 115 × 51.2 mm。"]),
(59,'模型范围',["局部壁厚、孔位、紧固件和安装间隙为近似设计。","电池、主板、芯片、风扇和控制器内部为主要模块布局示意。","尺寸数字描述此版模型，不能代替制造方工程资料。"]),
(102,'验证文件',["15 轮设计迭代和每轮模型保存在 iterations/。","15_interference_audit.json：完整实体干涉核验。","parameter_edit_test.json、rebuild_verification.json：参数与源码重建。","export_roundtrip_audit.json、drawing_checks.json：导出回读与尺寸检查。"]),
(157,'参考来源',["Nintendo 官方产品规格、Joy-Con/底座规格、部位图和数字手册。","iFixit 2025 年 Nintendo Switch 2 现场拆解：主要内部模块布局。","完整链接与尺寸依据见 references/SOURCES.md。"])
]:
...
(OUT/'reports/drawing_checks.json').write_text(json.dumps({'page_count':12,'dimensions':CHECKS,'model_envelope':M['published_handheld_mm'],'method':'visible-line projection of the final BRep; coordinates projected with the same rotation as dimension endpoints; section regions from planar BRep slices; strap front uses its top planar face and cord midplane silhouette discretized with 0.015 mm deflection','pass':all(c['value_mm']>0 for c in CHECKS)},ensure_ascii=False,indent=2))
