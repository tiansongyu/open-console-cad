...
for i,o in enumerate(C.values(),1):
    ...
    o.PartNumber=f'SW-{i:04d}'
    label=re.sub(r'\d{3,}$','',o.Label).replace('主机框架原坯','主机框架').replace(' · 原坯','')
    o.Label=o.PartNumber+' · '+label
visible(HANDHELD_GROUPS);camera((-.45,-.4,2),target=(0,0,7),span=165)
D.Label='Nintendo Switch · 完整套装 · 参数化'
D.saveAs(str(OUT/'Switch_Complete.FCStd'))
FINAL_VIEWS={}
...
('top',{'assemblies':HANDHELD_GROUPS,'normal':(0,1,.12),'up':(0,0,1),'target':(0,48,7),'span':50,'size':(2400,650)}),
('bottom',{'assemblies':HANDHELD_GROUPS,'normal':(0,-1,.12),'up':(0,0,1),'target':(0,-48,7),'span':50,'size':(2400,650)}),
('internal',{'assemblies':HANDHELD_GROUPS,'normal':(0,0,-1),'exclude':['TabletRear','Kickstand','StandFoot','RearNintendoMark','RearModelMark','RearEMIShield','JoyLRear','JoyRRear']}),
('cooling',{'assemblies':HANDHELD_GROUPS,'normal':(0,0,-1),'exclude':['TabletRear','Kickstand','StandFoot','RearNintendoMark','RearModelMark','RearEMIShield'],'target':(38,16,5),'span':94,'size':(1500,1400)}),
('usb_detail',{'assemblies':HANDHELD_GROUPS,'normal':(.2,-1,.18),'up':(0,0,1),'target':(0,-50,7),'span':14,'size':(1500,850)}),
('dock',{'assemblies':['Dock','DockInternal'],'normal':(-.65,.3,2),'target':(0,DOCK_Y,0),'span':150}),
('dock_open',{'assemblies':['Dock','DockInternal'],'normal':(-.35,.2,-2),'target':(0,DOCK_Y,0),'span':150,'exclude':['DockBackCover']}),
('grip',{'assemblies':['Grip'],'normal':(-.6,-.25,2),'target':(GRIP_X,GRIP_Y,0),'span':145}),
('straps',{'assemblies':['StrapL','StrapR'],'target':(0,-355,6),'span':205})]
for name,options in settings:FINAL_VIEWS[name]=snap('final_'+name,**options)
visible(HANDHELD_GROUPS);camera((-.45,-.4,2),target=(0,0,7),span=165);D.save()
# One independent exploded document holds all the kit parts with traceable offsets.
E=App.newDocument('SwitchExploded');E.Label='Nintendo Switch · 分层拆解'
EC={};EG={};OFFSET={}
...
E.saveAs(str(OUT/'Switch_Exploded.FCStd'))
# Explicit object lists prevent construction history from leaking into exports.
ImportGui.export(list(C.values()),str(OUT/'Switch_FullKit.step'))
ImportGui.export([o for o in C.values() if o.Assembly in HANDHELD_GROUPS],str(OUT/'Switch_Handheld.step'))
ImportGui.export([o for o in C.values() if o.Assembly in ['Dock','DockInternal']],str(OUT/'Switch_Dock.step'))
ImportGui.export(list(EC.values()),str(OUT/'Switch_Exploded.step'))
...
manifest={'project':'Nintendo Switch standard HAC-001 study','design_iterations':14,'physical_components':len(C),'solids':sum(len(o.Shape.Solids) for o in C.values()),'assemblies':dict(Counter(o.Assembly for o in C.values())),'handheld_groups':HANDHELD_GROUPS,'native_files':['Switch_Complete.FCStd','Switch_Exploded.FCStd'],'step_files':['Switch_FullKit.step','Switch_Handheld.step','Switch_Dock.step','Switch_Exploded.step'],'views':FINAL_VIEWS,'objects':rows,'exploded_objects':erows,'offsets':OFFSET,'published_handheld_mm':[239,102,28.4],'published_main_body_depth_mm':13.9,'published_dock_mm':[173,104,54],'fidelity':'Published overall envelopes; local dimensions approximate; major internals schematic'}
