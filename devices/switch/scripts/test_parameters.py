...
before={k:[C[k].Shape.Volume,C[k].Shape.BoundBox.ZMin,C[k].Shape.BoundBox.ZMax] for k in keys}
PARAM.set(cell,'9.3 mm');D.recompute()
changed={k:[C[k].Shape.Volume,C[k].Shape.BoundBox.ZMin,C[k].Shape.BoundBox.ZMax] for k in keys}
invalid_during=[o.Name for o in D.Objects if 'Invalid' in o.State]
PARAM.set(cell,old);D.recompute()
after={k:[C[k].Shape.Volume,C[k].Shape.BoundBox.ZMin,C[k].Shape.BoundBox.ZMax] for k in keys}
...
RESULT={'pass':True,'parameter':'StickProjection','original':old,'tested':'9.3 mm','keys':keys,'before':before,'changed':changed,'restored':after,'invalid_during':invalid_during,'invalid_after':invalid_after}
