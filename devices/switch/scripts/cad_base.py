...
ROOT = Path((globals().get('PROJECT_ROOT') or str(Path(__file__).resolve().parents[1])))
OUT = Path(globals().get('PROJECT_OUTPUT',ROOT / 'output'))
for name in ['iterations','previews','reports']:
