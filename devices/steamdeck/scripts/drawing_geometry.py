...
left_pad=[k for k in starts('Trackpad','Haptic') if '-1' in k]
...
v=s.view_fit('FaceButtons',starts('Button'),(280,142,86,48),(0,0,1));d.dimension_box(s,v,['ButtonA'],'x',132);caption(s,v,'ABXY 面按键',207);s.save()
...
v=s.view_fit('MainboardDetail',[k for k in d.keys_for(['Mainboard','Storage']) if k not in ['EMIShield','SSDShield','MicroSDCarrier'] and not k.startswith('MicroSD')],(20,29,207,120),(0,0,-1));d.dimension_box(s,v,['SSDPCB'],'y',12);caption(s,v,'APU / LPDDR5 / M.2 2230',162)
v=s.view_fit('BatteryDetail',d.keys_for(['Battery']),(252,40,126,103),(0,0,-1));d.dimension_box(s,v,['BatteryPouch'],'x',25);d.dimension_box(s,v,['BatteryPouch'],'y',391);caption(s,v,'40 Wh / 5200 mAh 电池布局',162)
s.text(14,186,'外观封装、板形、接点和电芯尺寸为近似值；不包含真实线路与制造公差。');s.text(14,201,'原始机型为 2022 LCD，不引入 OLED 或后期黑色主板罩结构。');s.save()

s=d.Sheet(8,'离心风机、热管与排风鳍片',['单风机、扁平铜热管、APU 冷板与排风鳍片分别建模。','散热件的壁厚、叶片与接触间隙为结构示意，不表示实际热性能。'])
thermal=[k for k in d.keys_for(['Thermal']) if k not in ['IntakeMesh','FanCable','FanConnector']]
...
v=s.view_fit('TriggerDetail',[k for k in starts('Trigger') if '-1' in k],(23,151,144,39),(.6,-.5,-1));caption(s,v,'单侧后扳机',207)
v=s.view_fit('RearKeys',[k for k in starts('RearPaddle','RearKey','RearButton') if '-1' in k],(229,151,143,39),(.6,-.5,-1));caption(s,v,'曲面背键与传动件',207);s.save()
...
v=s.view_fit('AdapterDetail',d.keys_for(['Accessories']),(215,136,165,58),(.3,-.5,1));d.dimension_box(s,v,['AdapterBody'],'x',125);caption(s,v,'独立 USB-C 电源附件',207);s.save()
